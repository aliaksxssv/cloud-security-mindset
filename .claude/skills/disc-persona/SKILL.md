...
The personas catalog holds four full persona prompt blocks plus shared servant-leadership stance and guardrails blocks. Loading it into main context wastes tokens on three personas the user did not pick. The workflow below routes both the catalog scan and the materialization of the chosen persona through subagents so **only the one adopted block ever enters main context**.
...
> 3. The section titled `## Guardrails (appended to every materialized persona file, verbatim)` — its body only (skip the parenthetical in the heading; include everything under it).
>
...
>
> <servant-leadership stance body, verbatim>
>
> # Guardrails
>
